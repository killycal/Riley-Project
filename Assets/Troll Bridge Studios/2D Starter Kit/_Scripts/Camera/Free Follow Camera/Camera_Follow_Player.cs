...
namespace TrollBridge {
	...
	public class Camera_Follow_Player : MonoBehaviour {
		...
		void Awake(){
			...
			Grid_Helper.helper.SetCameraRatios((float)cameraWidth, (float)cameraHeight, Screen.width, Screen.height);
		...
		void DebugCheck(){
			...
			if(cameraWidth <= 0){
				Grid_Helper.helper.DebugErrorCheck(2, this.GetType(), gameObject);
			}
			// IF the user forgot to put in the Cameras Height.
			if(cameraHeight <= 0){
				Grid_Helper.helper.DebugErrorCheck(3, this.GetType(), gameObject);
			}
			...
			if(bottomCameraBorder == null && canCameraMove){
				Grid_Helper.helper.DebugErrorCheck(5, this.GetType(), gameObject);
			}
			// IF the Top Camera Border is not setup with a GameObject AND we have a moving camera.
			if(topCameraBorder == null && canCameraMove){
				Grid_Helper.helper.DebugErrorCheck(6, this.GetType(), gameObject);
			}
			...
			if(leftCameraBorder == null && canCameraMove){
				Grid_Helper.helper.DebugErrorCheck(7, this.GetType(), gameObject);
			}
			...
			if(rightCameraBorder == null && canCameraMove){
				Grid_Helper.helper.DebugErrorCheck(8, this.GetType(), gameObject);
			}
			// IF the user mixed up the Top Camera Border and the Bottom Camera Border.
			if(bb > tb){
				Grid_Helper.helper.DebugErrorCheck(9, this.GetType(), gameObject);
			}
			// IF the user mixed up the Left Camera Border and the Right Camera Border.
			if(lb > rb){
				Grid_Helper.helper.DebugErrorCheck(10, this.GetType(), gameObject);
			}
		}
