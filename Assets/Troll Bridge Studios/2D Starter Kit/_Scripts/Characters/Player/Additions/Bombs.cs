		...
		void Update(){
			...
			if (Input.GetKeyDown (bombKey)) {
				...
				if (bombAmount > 0) {
					...
					Grid_Helper.helper.SpawnObject (bomb, playerAnimator.transform.position + new Vector3 (x, y, 0f), Quaternion.identity, playerAnimator.gameObject);
					// Play the sound of dropping a bomb.
					Grid_Helper.soundManager.PlaySound (placeBombSound);
					// Subtract a Bomb.
					AddSubtractBomb (-1);
				}
			}
