	...
	public class Player_Manager : Character, Can_Take_Damage, Can_Attack {
		...
		public void Attack(string animationNameValue, AudioClip clip){
			...
			if (!CharacterAnimator.GetBool (animationNameValue)) 
			{
				...
				Grid_Helper.soundManager.PlaySound (clip);
		...
		private void PlayerLookDirection(){
			if(ClosestAKD != null){
				...
				if(FourDirAnim){
					Grid_Helper.helper.FourDirectionAnimation(focTransform.position.x - characterEntity.transform.position.x, 
						focTransform.position.y - characterEntity.transform.position.y,
						false,
						CharacterAnimator);
				}else if(EightDirAnim){
					Grid_Helper.helper.EightDirectionAnimation(focTransform.position.x - characterEntity.transform.position.x, 
						focTransform.position.y - characterEntity.transform.position.y,
						false,
						CharacterAnimator);
				}
		...
		private void Death(){
			...
			if(DieSound != null){
				// Play the die sound.
				Grid_Helper.soundManager.PlaySound (DieSound, transform.position, 1f, 1f);
			}
		...
		private void Hit(Transform otherTransform, float joltAmount){
			...
			if(GetHitSound != null){
				Grid_Helper.soundManager.PlaySound (GetHitSound, transform.position, 1f, 1f);
			}
