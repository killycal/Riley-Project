		...
		void PlayAnimation(float hor, float vert){
			...
			if (_playerManager.CharacterAnimator != null) {
				...
				if (_playerManager.FourDirAnim) {
					// Play animations.
					Grid_Helper.helper.FourDirectionAnimation (hor * _playerManager.PlayerInvertX, vert * _playerManager.PlayerInvertY, _playerManager.CharacterAnimator);
				} else if (_playerManager.EightDirAnim) {
					// Play animation.
					Grid_Helper.helper.EightDirectionAnimation (hor * _playerManager.PlayerInvertX, vert * _playerManager.PlayerInvertY, _playerManager.CharacterAnimator);
				}
			}
		}
	}
