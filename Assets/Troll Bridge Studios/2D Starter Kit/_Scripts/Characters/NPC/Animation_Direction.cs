...
namespace TrollBridge {
	...
	public class Animation_Direction : MonoBehaviour {
		...
		private void PlayAnimation(float hor, float vert){
			...
			if (_fourDirAnim) {
				// Play animations.
				Grid_Helper.helper.FourDirectionAnimation (hor, vert, _animator);
			} else if (_eightDirAnim) {
				// Play animation.
				Grid_Helper.helper.EightDirectionAnimation (hor, vert, _animator);
			} else if (FlipToFace) {
		...
		private void NPCLookDirection(){
			...
			if(_fourDirAnim){
				...
				Grid_Helper.helper.FourDirectionAnimation(focTransform.position.x - _transform.position.x, 
					focTransform.position.y - _transform.position.y,
					false,
					_animator);
			}else if(_eightDirAnim){
				// Make this gameobjets animation face in the direction desired.
				Grid_Helper.helper.EightDirectionAnimation(focTransform.position.x - _transform.position.x, 
					focTransform.position.y - _transform.position.y,
					false,
					_animator);
			}
