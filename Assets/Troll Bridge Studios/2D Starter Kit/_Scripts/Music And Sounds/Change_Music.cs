...
namespace TrollBridge {
	...
	public class Change_Music : MonoBehaviour {
	    ...
	    void Awake()
	    {
	        ...
	        {
	            // Play the BG Music.
				Grid_Helper.soundManager.PlayBGMusic(NameMusic[SceneManager.GetActiveScene().buildIndex].BGMusic);
	        }
	    }

		void OnLoadedLevel(Scene scene, LoadSceneMode mode){
			// IF a different sound is going to be played.
			if(Grid_Helper.soundManager.bgMusicSource.clip != NameMusic[scene.buildIndex].BGMusic){
				// Stop all Coroutines.
				StopAllCoroutines();
				...
				if(fadeTransition){
					...
					if (!isFadeRunning) {
						...
						savedVolume = Grid_Helper.soundManager.bgMusicSource.volume;
						currentVolume = savedVolume;
					} else {
						currentVolume = Grid_Helper.soundManager.bgMusicSource.volume;
					}
				...
				// Play the BG Music.
				Grid_Helper.soundManager.PlayBGMusic(NameMusic[scene.buildIndex].BGMusic);
			}
		}
		...
		private IEnumerator FadeBGMusic(){
			...
			for(float x = 0f; x <= 1.0f; x += Time.deltaTime / (fadeTime/2f)){
				...
				Grid_Helper.soundManager.bgMusicSource.volume = Mathf.SmoothStep(currentVolume, 0f, x);
				yield return null;
			}
			...
			Grid_Helper.soundManager.bgMusicSource.volume = Mathf.SmoothStep(currentVolume, 0f, 1f);
			// Play the BG Music.
			Grid_Helper.soundManager.PlayBGMusic(NameMusic[currentLevel].BGMusic);
			// Fade the next song in.
			for(float x = 0f; x <= 1.0f; x += Time.deltaTime / (fadeTime/2f)){
				// Smooth step the volume back to savedVolume.
				Grid_Helper.soundManager.bgMusicSource.volume = Mathf.SmoothStep(0f, savedVolume, x);
				yield return null;
			}
			// Make sure the sound change is consistent so we make sure we put a 1f for our time incase our for loop x didnt exactly land on 1f.
			Grid_Helper.soundManager.bgMusicSource.volume = Mathf.SmoothStep(0f, savedVolume, 1f);
