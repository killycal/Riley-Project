	...
	public class Area_Dialogue : MonoBehaviour {
		...
		void DebugCheck(){
			...
			if(showAreaInScene && (rangeCollider == null)){
				Grid_Helper.helper.DebugErrorCheck(70, this.GetType(), gameObject);
			}
			// IF the user didnt set a dialogue box gameobject to be shown.
			if(dialogueBox == null){
				Grid_Helper.helper.DebugErrorCheck(71, this.GetType(), gameObject);
			}
			// IF the user didn't set any dialogue for the NPC.
			if(dialogue.Length == 0){
				Grid_Helper.helper.DebugErrorCheck(72, this.GetType(), gameObject);
			}
			// IF the user didnt set a Dialogue componenet on the dialogueBox.
			if(dialogueComponent == null){
				Grid_Helper.helper.DebugErrorCheck (79, this.GetType(), gameObject);
			}
		}
