...
namespace TrollBridge {
	...
	public class Terrain_Pushable : MonoBehaviour {
		...
		void PlayPushSound(){
			...
			if(soundSource == null){
				soundSource = Grid_Helper.soundManager.PlaySound(soundClip, _transform.position, minPitch, maxPitch);
			}
		}
		...
		void DebugCheck(){
			...
			if(moveSpeed == 0){
				Grid_Helper.helper.DebugErrorCheck(35, this.GetType(), gameObject);
			}
			// IF the Transform up, down, left and right are set to null.
			if(up == null && down == null && left == null && right == null){
				Grid_Helper.helper.DebugErrorCheck(36, this.GetType(), gameObject);
			}
		}
	}
}
