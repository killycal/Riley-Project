...
namespace TrollBridge {
	...
	public class Loot : MonoBehaviour {
		...
		private void DropItem(GameObject loot){
			// Spawn the drop.
			GameObject item = Grid_Helper.helper.SpawnObject (loot, GetComponentInParent<Character>().characterEntity.transform.position, Quaternion.identity, GetComponentInParent<Character>().characterEntity);
			// IF there is a Rigidbody2D.
			if(item.GetComponent<Rigidbody2D>() != null){
				// Launch the loot around.
				Grid_Helper.helper.LaunchItem (item, -1f, 1f, -1f, 1f);
			}
		}
	}
}
