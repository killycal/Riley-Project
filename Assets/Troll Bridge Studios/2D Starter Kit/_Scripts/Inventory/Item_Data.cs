	...
	public class Item_Data : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerEnterHandler, IPointerExitHandler, IPointerDownHandler, IPointerUpHandler {
		...
		public void OnPointerEnter(PointerEventData data)
		{
			...
			if(Grid_Helper.tooltip != null){
				// Show the tooltip.
				Grid_Helper.tooltip.Activate (item);
			}
		}
		...
		public void OnPointerExit(PointerEventData data)
		{
			...
			if(Grid_Helper.tooltip != null){
				...
				Grid_Helper.tooltip.Deactivate ();
			}
		}
		...
		public void OnPointerUp(PointerEventData data)
		{
			...
			if(item != null && data.button == PointerEventData.InputButton.Right && !data.dragging)
			{
				...
				if (item.Type == "Consumable") 
				{
					...
					Grid_Helper.soundManager.PlaySound (item.UsedSound);
					if(amount - 1 == 0){
						// Clear the old slot.
		...
		public void OnEndDrag(PointerEventData data)
		{
			if (data.button == PointerEventData.InputButton.Left) 
			{
				...
				if (data.hovered.Count == 0) 
				{
					...
					GameObject goItem = Grid_Helper.helper.SpawnObject (Grid_Helper.setup.GetGameObjectPrefab (item.Title), Character_Manager.GetPlayer ().transform.position, Quaternion.identity, Character_Manager.GetPlayer (), RADIUS);
					// Launch the item in a random direction.
					Grid_Helper.helper.LaunchItemAwayFromPosition (goItem, Character_Manager.GetPlayer ().transform.position);
					// Clear the old slot.
					GetComponentInParent<Inventory>().items [slotNumber] = new Item ();
