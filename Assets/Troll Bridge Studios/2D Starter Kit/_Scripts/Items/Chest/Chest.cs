...
namespace TrollBridge {
	...
	public class Chest : MonoBehaviour {
		...
		public void OpenChest(GameObject collidingObject){
			...
			Grid_Helper.soundManager.PlaySound (openChestSound);
			// Spawn the loot from the chest.
			GameObject chestGO = Grid_Helper.helper.SpawnObject (chestLoot, transform.position, Quaternion.identity, gameObject);
			// Launch the item in a direction away from the colliding object.
			Grid_Helper.helper.LaunchItemAwayFromPosition (chestGO, collidingObject.transform.position);
