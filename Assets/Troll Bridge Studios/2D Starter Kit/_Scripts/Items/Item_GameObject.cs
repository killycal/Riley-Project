...
namespace TrollBridge {
	...
	public class Item_GameObject : MonoBehaviour {
		...
		private bool HandleInventory()
		{
			...
			Inventory inv = Grid_Helper.inventory;
			...
			if (stackable) {
				...
				if (inv.items.Contains (Grid_Helper.itemDataBase.FetchItemByID (id)) || inv.GetFreeSpots () >= 1) {
		...
		private void CheckState ()
		{
			...
			if (isPlaced) {
				...
				if (GetComponent<State_Transform> () != null) {
					...
					Grid_Helper.stateManager.Unregister (gameObject);
				}
