		...
		public void Load()
		{
			...
			for (int i = 0; i < data.objectData.Count; i++) {
				...
				if (gObject == null) {
					...
					gObject = Instantiate (Grid_Helper.setup.GetGameObjectPrefab (data.objectData[i].name));
				}
				...
				if (gObject.GetComponent<Item_Data> () == null) {
					...
					if (!String.IsNullOrEmpty (data.objectData[i].spriteName)) {
						...
						gObject.GetComponent<SpriteRenderer> ().sprite = Grid_Helper.setup.GetSprite (data.objectData[i].spriteName);
